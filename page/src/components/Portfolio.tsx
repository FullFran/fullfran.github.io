...
type PortfolioData = { about: string; experience: string; skills: string; contact: string; help: string; };
...
const DRAFTS_KEY = 'fullfran_blog_drafts_v1';
...
const App: React.FC<PortfolioProps> = ({ posts = [], onExitTerminal }) => {
  const [data, setData] = React.useState<PortfolioData>(DEFAULT_DATA);
  // Blog posts come from markdown (single source of truth), read-only in the terminal.
  const markdownPosts: BlogPost[] = React.useMemo(
    () =>
      posts.map((p) => ({
        id: p.slug,
        title: `${p.slug}.md`,
  ...
  // Admin can still append client-side drafts on top via `:touch` (persisted in localStorage).
  const [drafts, setDrafts] = React.useState<BlogPost[]>([]);
  const blog = React.useMemo(() => [...markdownPosts, ...drafts], [markdownPosts, drafts]);
  ...
  const handleContentChange = (newContent: string) => {
    // Markdown posts are read-only in the terminal; only client-side drafts are editable.
    if (activeTab === 'blog' && activeBlogPostId) {
      if (drafts.some(p => p.id === activeBlogPostId)) {
        persistDrafts(drafts.map(p => p.id === activeBlogPostId ? { ...p, content: newContent } : p));
      }
      // TODO: markdown-sourced posts are not editable client-side (source of truth is src/content/blog).
    }
  ...
  const handleCreatePost = (filename: string) => { if (!isAdmin || !filename) return; persistDrafts([...drafts, { id: filename.replace(/\s/g,'-').toLowerCase(), title: filename, date: new Date().toISOString().split('T')[0], content: `# ${filename}` }]); };
  const handleDeletePost = (id: string) => { if (!isAdmin || !window.confirm(`Delete ${id}?`)) return; if (!drafts.some(p => p.id === id)) { alert('Markdown posts are read-only. Only drafts can be deleted.'); return; } persistDrafts(drafts.filter((p) => p.id !== id)); if (activeBlogPostId === id) setActiveBlogPostId(null); };
  ...
  }, [mode, commandBuffer, showAdminLogin, isAdmin, activeTab, activeBlogPostId, data, drafts, markdownPosts, cursor, count, gBuffer]);
